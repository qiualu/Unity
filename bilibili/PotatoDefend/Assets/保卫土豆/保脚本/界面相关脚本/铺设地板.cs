...
public class 铺设地板 : MonoBehaviour
{
    ...
    public static 铺设地板 铺设地板实例 { get; private set; }
    ...
    // 已点击地板列表
    private List<保地板类> 已点击地板 = new List<保地板类>();
    ...
    public void 关卡加载()
    {
        ...
        Debug.Log($"关卡加载类 : 关卡加载！{保卫管理.实例.游戏状态}");
    ...
    public void 演示路图()
    {
        ...
        foreach (var 路径点 in 关卡.路径点列表)
        {
            ...
            //地板.地板id = x * 14 + y;
            保地板类 地板交互脚本 = 地板.GetComponent<保地板类>();
            if (地板交互脚本 != null)
            {
                // 给脚本里的地板id赋值
                地板交互脚本.地板id = x * 14 + y;
            ...
            {
                Debug.LogError("地板对象上没有挂载 保地板类 脚本！");
            }
    ...
    public void 铺设一个测试版本()
    {
        ...
        int x = 1;  // 假设 路径点.w 是 float 类型（如 5.0f）
        int y = 1;  // 直接强制转换为 int
        ...
        地板.name = "测试版本";  // 核心修改：将路径点的名字赋值给对象名
        //地板.地板id = x * 14 + y;
        保地板类 地板交互脚本 = 地板.GetComponent<保地板类>();
